...
let sendData = function (metric, tags, value) {
    ...
    if (process.env.INSIGHTS_STORAGE === "elasticsearch") {
        ...
        return axios({
            ...
            console.log("[METRICS-DRUID] Error in sending message to elastic search :: ", err.stack || err.message);
        })
    }
    ...
    if (metric === 'events.bots.custom') {
        ...
        return axios(options).catch(err => {
            console.log("[METRICS-DRUID] Error in sending message to kafka through /publish :: ", err.stack || err.message);
        })
    }
    axios({
        url: indiaUrl,
