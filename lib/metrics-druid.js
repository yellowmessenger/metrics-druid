...
let sendData = function (metric, tags, value) {
    if (Object.keys(tags).length > 0 && process.env.NODE_ENV === "deployment") {
    ...
    }else if (true || Object.keys(tags).length > 0 && (process.env.NODE_ENV === "production" || process.env.NODE_ENV === "docker")) {
        let metricData = {
            "metric": metric,
