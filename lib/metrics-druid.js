...
axiosRetry(axios, {
    ...
    retryCondition: (response) => {
        const shouldBeRetried = response.code === 'ECONNABORTED' || axiosRetry.isNetworkError(response);
...
let sendData = function (metric, tags, value) {
    ...
    if (process.env.INSIGHTS_STORAGE === "elasticsearch") {
        let index = `botmetrics-${metric}`;
        if (metric === 'events.bots.custom') {
            const botId = tags.user || tags.botId;
        ...
        return axios({
            url: `${esURL}/${index}/_doc`,
            method: "POST",
            data: metricData,
            timeout
        }).catch(err => {
            console.log("[METRICS-DRUID] Error in sending message to elastic search :: ", err.stack, err.config);
        })
    }
    ...
    if (metric === 'events.bots.custom') {
        ...
        return axios(options).catch(err => {
            console.log("[METRICS-DRUID] Error in sending message to kafka through /publish :: ", err.stack, err.config);
        })
    }
    axios({
        url: indiaUrl,
